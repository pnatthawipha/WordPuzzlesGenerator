...
def generate_grid(row, col, wsdictionary, fromlist):
    ...
    while not fillgrid:
        ...
        for word in words_with_directions:
            ...
            notfoundcount = 0
            while not found:
                i, j, found = check_direction(words_with_directions[word][0],
                                              matrix, row, col, word)
                if found:
                    end_i, end_j = matrix.add_word(word, i, j,
                                                   words_with_directions[word][0])
